...
{
    __m128d numerator = _mm_sub_pd(exp(x), exp(negate(x)));
...
{
    __m128d numerator = _mm_add_pd(exp(x), exp(negate(x)));
    return _mm_mul_pd(numerator, _mm_set1_pd(0.5));
}
...
{
    __m128d denominator = _mm_add_pd(exp(x), exp(negate(x)));
...
{
    __m128d denominator = _mm_sub_pd(exp(x), exp(negate(x)));
    return _mm_div_pd(_mm_set1_pd(0.5), denominator);
}
